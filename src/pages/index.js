        ...
        {userData && (
          <div className="mt-10 bg-gray-800 p-6 rounded-lg shadow-md text-left">
            <div className="flex items-center mb-4">
              ...
              <div>
                ...
                <p className="text-gray-400">
                  {language === 'tr' ? 'Kullanıcı ID:' : 'User ID:'} {userData.id}
                </p>
              </div>
            </div>
            {userData.badges && (
            ...
            <p className="text-gray-400 mb-2">
              ...
              </span> {new Date(userData.created_at).toLocaleString(language)}
            </p>
            {userData.banner_color && (
              <p className="text-gray-400 mb-2">
                ...
                <span style={{ backgroundColor: userData.banner_color }} className="ml-2 p-1 rounded">{userData.banner_color}</span>
              </p>
            )}
          </div>
        )}
        ...
        {showModal && (
          <div className="fixed top-0 left-0 w-full h-full bg-gray-900 bg-opacity-50 flex justify-center items-center">
            <div className="bg-white p-4 rounded-lg shadow-lg">
              <h2 className="text-xl font-bold mb-4 text-gray-700">{language === 'tr' ? 'Captcha Doğrulaması' : 'Captcha Verification'}</h2>
              <p className="mb-4 text-gray-700" >{captchaNum1} {captchaOperation} {captchaNum2} = ?</p>
              <input
                type="number"
               // value={captchaAnswer}
                onChange={(e) => setCaptchaAnswer(parseInt(e.target.value))}
